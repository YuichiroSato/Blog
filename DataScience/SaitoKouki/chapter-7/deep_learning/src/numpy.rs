...
impl NumpyArray {
    ...
    pub fn addv_row(&self, v: &NumpyVector) -> Self {
        let mut arr = NumpyArray::new(self.row, self.column);
        for r in 0..self.row {
            for c in 0..self.column {
                arr.data[r][c] = self.data[r][c] + v.data[r];
            }
        }
        arr
    }
    ...
    pub fn max_in(&self, start_row: usize, start_column: usize, row_span: usize, column_span: usize) -> (f32, (usize, usize)) {
        let mut x = std::f32::NEG_INFINITY;
        let mut i = (0, 0);
        for r in start_row..(start_row + row_span) {
            for c in start_column..(start_column + column_span) {
                if x < self.data[r][c] {
                    x = self.data[r][c];
                    i = (r, c)
                }
...
impl NumpyVector {
    ...
    pub fn to_array(&self, row_size: usize) -> NumpyArray {
        let column_size = (self.data.len() as f32 / row_size as f32) as usize;
        let mut arr = vec![vec![0.0; row_size]; column_size];
        for i in 0..column_size {
            arr[i] = self.data[(i * row_size)..((i + 1) * row_size)].to_vec();
        }
